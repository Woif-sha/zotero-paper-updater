...
## 将全局 Skill 接入 llm-for-zotero

`llm-for-zotero` 的 Codex 运行时从 Zotero 数据目录下的
`agent-runtime/profile-.../.agents/skills` 发现 Skill。本机可把
`~/.agents/skills/.skills-list.json` 中由 `skills-updater` 登记的全局 Skill
逐个链接进去：

```powershell
pwsh -File .\scripts\sync-global-skills-to-runtime.ps1 -AsJson
```

脚本创建的是真正的目录符号链接，因此全局 Skill 更新后，运行时立即看到同一份文件，
无需复制。它不会替换整个运行时 `skills` 目录，也不会覆盖插件自带 Skill；若同名目录
已经存在且不是指向同一来源的链接，脚本会在任何写入前以退出码 `2` 停止。

当前 `profile-df7401f1` 后缀由 Zotero profile 目录路径计算，并不包含插件版本，普通
插件升级不会改变它。切换/重命名 Zotero profile 或上游改变布局时，运行时目录仍可能
变化。脚本因此不硬编码 profile 后缀，而是在 `agent-runtime` 下递归发现当前所有
`.agents/skills`；插件升级或切换 profile 后重新运行即可补齐链接。若上游连
`agent-runtime` 根目录也改变，可显式传入 `-RuntimeRoot`。

只链接部分已登记 Skill：

```powershell
pwsh -File .\scripts\sync-global-skills-to-runtime.ps1 `
  -SkillName zotero-paper-updater,skills-updater
```
...
│   ├── sync-global-skills-to-runtime.ps1
